...
from bin.hummingbot import UIStartListener, UIExitListener, detect_available_port
from bin.datradingbot import DATradingBot
...
async def quick_start(args: argparse.Namespace, secrets_manager: BaseSecretsManager):
    ...
    start_listener: UIStartListener = UIStartListener(hb, is_script=is_script, script_config=script_config,
                                                      is_quickstart=True)
    exit_listener: UIExitListener = UIExitListener(hb, is_script=is_script, script_config=script_config,
                                                      is_quickstart=True)
    hb.app.add_listener(HummingbotUIEvent.Start, start_listener)
    hb.app.add_listener(HummingbotUIEvent.Exit, exit_listener)
...
def main():
    with open("hummingbot/client/config/is_da.txt", 'w') as DA:
        DA.write("0")
    ...
    is_da = 0
    with open("hummingbot/client/config/is_da.txt", 'r') as DA:
        is_da = eval(DA.readline())
    
    if is_da == 1:
        da_trader = DATradingBot()
        da_trader.run()
    else:
        print("Hello, world!")
